...
public class QuestionActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        chronometer.setOnChronometerTickListener(new Chronometer.OnChronometerTickListener() {
            ...
            public void onChronometerTick(Chronometer chronometer) {
                // 此处时间设定为1.5小时
                if (SystemClock.elapsedRealtime() - chronometer.getBase() > 1.5 * 60 * 60 * 1000) {
                    Toast.makeText(QuestionActivity.this, "考试时间到", Toast.LENGTH_SHORT).show();
                    saveExam();
                }
            }
        });
        ...
        if (tab == MyTag.QUE)
            cursor = ToolHelper.loadDB(this,
                    // select que.* from que where source ='2022考研408模拟试题（一）' order by type
                    // select que.* from que where kind ='数据结构' order by type
    ...
    private void saveExam() {
        ...
        String myDate = ft.format(date);
        String title = source + "\n" + "(" + content + ")";
        ToolHelper.excuteDB(this, "insert into exam (_id,uid,title,examTime,score,examDate) values (" + null
                + "," + UserInfo.userID + ",'" + title + "','" + myTime + "'," + score + ",'" + myDate + "')");
